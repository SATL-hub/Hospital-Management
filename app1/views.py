...
def profileedit(r):
    if 'id' in r.session:
        ...
        if r.method == 'POST':
            ...
            pic = r.FILES.get('img')
            print("piccc",pic)
            resumee = r.FILES.get('file')
            print("resumee",resumee)
            if pic == None:
                usr.save()
                try:
                    re=resume.objects.create(name=usr.name,file_resume=resumee)
                    re.save()
                except Exception as e:
                    print("error",e)
            else:
...
def forgot_password(request):
    if request.method == 'POST':
        ...
        reset_link = f'http://http://127.0.0.1:8001/reset/{token}'
        try:
           
            send_mail('hii','Reset Your Password',settings.EMAIL_HOST_USER,['gouriemaildemo@gmail.com'])
        ...
        if r.method == 'POST':
            if r.POST.get('save') == 'save':
                ...
                if pro:
                    ...
                    cr = profile.objects.create(user=usr, fname=fname, lname=lname, email=email, phone=phone,
                ...
                print("cityyyyyyyy",r.POST.get('city'))
                neworder.state = r.POST.get('state')
                print("stateeeee",neworder.state)
                ...
                neworder.save()
                print("hiiiiiiiiiiiiiiiiiihhhhhhhhhhhhh")
